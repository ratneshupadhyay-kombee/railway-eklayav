...
use App\Helper;
use App\Jobs\ExportDispenserTable;
...
final class Table extends PowerGridComponent
{
    ...
    public string $sortField = 'dispensers.id';
    ...
    public function __construct()
    {
        ...
        $this->tableName = __('messages.dispenser.listing.tableName');
    ...
    public function exportData()
    {
        try {
            ...
            $exportClass = ExportDispenserTable::class;
            $headingColumn = 'Number';
            $batchName = 'Export Dispenser Table';
            $downloadPrefixFileName = 'DispenserReports_';
            $extraParam = [];
            ...
            logger()->error('App\Livewire\DispenserTable: exportData: Throwable', ['Message' => $e->getMessage(), 'TraceAsString' => $e->getTraceAsString()]);
            session()->flash('error', __('messages.dispenser.messages.common_error_message'));
    ...
    public function header(): array
    {
        ...
        if (Gate::allows('add-dispenser')) {
            $headerArray[] = Button::add('add-dispenser')
                ->slot('    <a href="/dispenser/create" title="Add New Dispenser" data-testid="add_new" class="flex items-center justify-center cursor-pointer" wire:navigate>
        ...
        if (Gate::allows('export-dispenser')) {
            $headerArray[] = Button::add('export-data')
                ->slot('<a href="javascript:void(0);" title="Export Dispenser" data-testid="export_button" wire:click="exportData" class="flex items-center justify-center" wire:loading.attr="disabled">
        ...
        if (Gate::allows('bulkDelete-dispenser')) {
    ...
    public function datasource(): Builder
    {
        ...
        return Dispenser::query()
            ...
            ->select([
                'dispensers.id', 'dispensers.number',
            ]);
    }
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('messages.dispenser.listing.id'), 'id')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.dispenser.listing.number'), 'number')
                ->sortable()
                ->searchable(),
            Column::make(__('messages.created_date'), 'created_at_formatted', 'created_at'),
            Column::action(__('messages.dispenser.listing.actions')),
        ];
    }
    ...
    public function filters(): array
    {
        return [
            ...
            Filter::inputText('number', 'dispensers.number')->operators(['contains']),
    ...
    {
        return $this->redirect('dispenser/' . $id . '/edit', navigate: true); // redirect to edit component
    }

    public function actions(Dispenser $row): array
    {
        $actions = [];

        if (Gate::allows('show-dispenser')) {
            $actions[] = Button::add('view')
                ...
                ->dispatchTo('dispenser.show', 'show-dispenser-info', ['id' => $row->id]);
        }

        if (Gate::allows('edit-dispenser')) {
        ...
        if (Gate::allows('delete-dispenser')) {
            $actions[] = Button::add('delete-dispenser')
                ...
                ->dispatchTo('dispenser.delete', 'delete-confirmation', ['ids' => [$row->id], 'tableName' => $this->tableName]);
        }
