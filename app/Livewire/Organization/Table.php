...
use App\Helper;
use App\Jobs\ExportOrganizationTable;
...
final class Table extends PowerGridComponent
{
    ...
    public function __construct()
    {
        ...
        $this->tableName = __('messages.organization.listing.tableName');
    ...
    public function exportData()
    {
        try {
            ...
            $exportClass = ExportOrganizationTable::class;
            $headingColumn = 'Name,OwnerName,ContactNumber,State,City,Pincode';
            $batchName = 'Export Organization Table';
            $downloadPrefixFileName = 'OrganizationReports_';
            $extraParam = [];
            ...
            logger()->error('App\Livewire\OrganizationTable: exportData: Throwable', ['Message' => $e->getMessage(), 'TraceAsString' => $e->getTraceAsString()]);
            session()->flash('error', __('messages.organization.messages.common_error_message'));
    ...
    public function header(): array
    {
        ...
        if (Gate::allows('add-organization')) {
            $headerArray[] = Button::add('add-organization')
                ->slot('    <a href="/organization/create" title="Add New Organization" data-testid="add_new" class="flex items-center justify-center cursor-pointer" wire:navigate>
        ...
        if (Gate::allows('export-organization')) {
            $headerArray[] = Button::add('export-data')
                ->slot('<a href="javascript:void(0);" title="Export Organization" data-testid="export_button" wire:click="exportData" class="flex items-center justify-center" wire:loading.attr="disabled">
        ...
        if (Gate::allows('bulkDelete-organization')) {
    ...
    public function datasource(): Builder
    {
        ...
        return Organization::query()
            ...
            ->select([
                'organizations.id', 'organizations.name', 'organizations.owner_name', 'organizations.contact_number', 'organizations.state', 'organizations.city', 'organizations.pincode',
            ]);
    }
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('messages.organization.listing.id'), 'id')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.organization.listing.name'), 'name')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.organization.listing.owner_name'), 'owner_name')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.organization.listing.contact_number'), 'contact_number')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.organization.listing.state'), 'state')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.organization.listing.city'), 'city')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.organization.listing.pincode'), 'pincode')
                ->sortable()
                ->searchable(),
            Column::make(__('messages.created_date'), 'created_at_formatted', 'created_at'),
            Column::action(__('messages.organization.listing.actions')),
        ];
    }
    ...
    public function filters(): array
    {
        return [
            ...
            Filter::inputText('name', 'organizations.name')->operators(['contains']),
            Filter::inputText('owner_name', 'organizations.owner_name')->operators(['contains']),

            Filter::inputText('state', 'organizations.state')->operators(['contains']),
            Filter::inputText('city', 'organizations.city')->operators(['contains']),
    ...
    {
        return $this->redirect('organization/' . $id . '/edit', navigate: true); // redirect to edit component
    }
    ...
    public function actions(Organization $row): array
    {
        ...
        if (Gate::allows('show-organization')) {
            $actions[] = Button::add('view')
                ...
                ->dispatchTo('organization.show', 'show-organization-info', ['id' => $row->id]);
        }
        ...
        if (Gate::allows('delete-organization')) {
            $actions[] = Button::add('delete-organization')
                ...
                ->dispatchTo('organization.delete', 'delete-confirmation', ['ids' => [$row->id], 'tableName' => $this->tableName]);
        }
