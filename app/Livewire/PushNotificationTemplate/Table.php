...
final class Table extends PowerGridComponent
{
    ...
    public string $sortField = 'push_notification_templates.id';
    ...
    public function __construct()
    {
        ...
        $this->tableName = __('messages.push_notification_template.listing.tableName');
    ...
    public function header(): array
    {
        ...
        if (Gate::allows('add-push-notification-template')) {
            $headerArray[] = Button::add('add-push-notification-template')
                ->slot('    <a href="/push-notification-template/create" title="Add New PushNotificationTemplate" data-testid="add_new" class="flex items-center justify-center cursor-pointer" wire:navigate>
        ...
        if (Gate::allows('export-push-notification-template')) {
            $headerArray[] = Button::add('export-data')
                ->slot('<a href="javascript:void(0);" title="Export PushNotificationTemplate" data-testid="export_button" wire:click="exportData" class="flex items-center justify-center" wire:loading.attr="disabled">
        ...
        if (Gate::allows('bulkDelete-push-notification-template')) {
            $headerArray[] = Button::add('bulk-delete')
                ...
                <div class="flex rounded-md ring-1 transition focus:ring-2
                    ...
                    title="Bulk Delete PushNotificationTemplates">
    ...
    {
        // Main query
        return PushNotificationTemplate::query()

            ->select([
                'push_notification_templates.id', 'push_notification_templates.label', 'push_notification_templates.title',
                DB::raw(
                    '(CASE
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('messages.push_notification_template.listing.id'), 'id')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.push_notification_template.listing.label'), 'label')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.push_notification_template.listing.title'), 'title')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.push_notification_template.listing.status'), 'status')
                ->sortable()
                ->searchable(),
            Column::make(__('messages.created_date'), 'created_at_formatted', 'created_at'),
            Column::action(__('messages.push_notification_template.listing.actions')),
        ];
    }
    ...
    public function filters(): array
    {
        return [
            ...
            Filter::inputText('label', 'push_notification_templates.label')->operators(['contains']),
            Filter::inputText('title', 'push_notification_templates.title')->operators(['contains']),
            Filter::select('status', 'status')
                ->dataSource(PushNotificationTemplate::status())
                ->optionLabel('label')
                ->optionValue('key'),
    ...
    {
        return $this->redirect('push-notification-template/' . $id . '/edit', navigate: true); // redirect to edit component
    }

    public function actions(PushNotificationTemplate $row): array
    {
        $actions = [];

        if (Gate::allows('show-push-notification-template')) {
            $actions[] = Button::add('view')
                ...
                ->dispatchTo('push-notification-template.show', 'show-pushnotificationtemplate-info', ['id' => $row->id]);
        }

        if (Gate::allows('edit-push-notification-template')) {
        ...
        if (Gate::allows('delete-push-notification-template')) {
            $actions[] = Button::add('delete-push-notification-template')
                ...
                ->dispatchTo('push-notification-template.delete', 'delete-confirmation', ['ids' => [$row->id], 'tableName' => $this->tableName]);
        }
