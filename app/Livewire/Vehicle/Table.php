...
use App\Helper;
use App\Jobs\ExportVehicleTable;
...
final class Table extends PowerGridComponent
{
    ...
    public string $sortField = 'vehicles.id';
    ...
    public function __construct()
    {
        ...
        $this->tableName = __('messages.vehicle.listing.tableName');
    ...
    public function exportData()
    {
        try {
            ...
            $exportClass = ExportVehicleTable::class;
            $headingColumn = 'User,VehicleNumber,FuelType,Status';
            $batchName = 'Export Vehicle Table';
            $downloadPrefixFileName = 'VehicleReports_';
            $extraParam = [];
            ...
            logger()->error('App\Livewire\VehicleTable: exportData: Throwable', ['Message' => $e->getMessage(), 'TraceAsString' => $e->getTraceAsString()]);
            session()->flash('error', __('messages.vehicle.messages.common_error_message'));
    ...
    public function header(): array
    {
        ...
        if (Gate::allows('add-vehicle')) {
            $headerArray[] = Button::add('add-vehicle')
                ->slot('    <a href="/vehicle/create" title="Add New Vehicle" data-testid="add_new" class="flex items-center justify-center cursor-pointer" wire:navigate>
        ...
        if (Gate::allows('export-vehicle')) {
            $headerArray[] = Button::add('export-data')
                ->slot('<a href="javascript:void(0);" title="Export Vehicle" data-testid="export_button" wire:click="exportData" class="flex items-center justify-center" wire:loading.attr="disabled">
        ...
        if (Gate::allows('bulkDelete-vehicle')) {
    ...
    public function datasource(): Builder
    {
        ...
        return Vehicle::query()
            ...
            ->select([
                'vehicles.id', 'users.first_name as user_first_name', 'vehicles.vehicle_number',
                DB::raw(
                    '(CASE
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make(__('messages.vehicle.listing.id'), 'id')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.vehicle.listing.users'), 'user_first_name')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.vehicle.listing.vehicle_number'), 'vehicle_number')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.vehicle.listing.fuel_type'), 'fuel_type')
                ->sortable()
                ->searchable(),

            Column::make(__('messages.vehicle.listing.status'), 'status')
                ->sortable()
                ->searchable(),
            Column::make(__('messages.created_date'), 'created_at_formatted', 'created_at'),
            Column::action(__('messages.vehicle.listing.actions')),
        ];
    }
    ...
    public function filters(): array
    {
        return [
            ...
            Filter::inputText('vehicle_number', 'vehicles.vehicle_number')->operators(['contains']),
            Filter::select('fuel_type', 'fuel_type')
                ->dataSource(Vehicle::fuel_type())
                ->optionLabel('label')
                ->optionValue('key'),
            Filter::select('status', 'status')
                ->dataSource(Vehicle::status())
                ->optionLabel('label')
                ->optionValue('key'),
    ...
    {
        return $this->redirect('vehicle/' . $id . '/edit', navigate: true); // redirect to edit component
    }
    ...
    public function actions(Vehicle $row): array
    {
        ...
        if (Gate::allows('show-vehicle')) {
            $actions[] = Button::add('view')
                ...
                ->dispatchTo('vehicle.show', 'show-vehicle-info', ['id' => $row->id]);
        }
        ...
        if (Gate::allows('delete-vehicle')) {
            $actions[] = Button::add('delete-vehicle')
                ...
                ->dispatchTo('vehicle.delete', 'delete-confirmation', ['ids' => [$row->id], 'tableName' => $this->tableName]);
        }
